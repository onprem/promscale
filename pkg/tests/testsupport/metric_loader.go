...
func (it *promIterator) loadBlockSamples() error {
	log.Info("loading blocks, total samples", it.blocks[it.curBlockIdx].Meta().Stats.NumSamples, "series", it.blocks[it.curBlockIdx].Meta().Stats.NumSeries)
...
func (si *sampleIngestor) ingestSamples(ingest IngestFunc) {
	...
	for i := 0; i < len(si.shards); i++ {
		go func(shard int) {
			...
			counter := 0
			for sample := range si.shards[shard] {
